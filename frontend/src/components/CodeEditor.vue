  ...
  if (editorView.value) {
    ...
    try {
      ...
      if (props.language === 'json') {
        ...
        formatted = formatPythonCode(doc)
      }
      editorView.value.dispatch({
        changes: {
          from: 0,
...
onMounted(() => {
  ...
  const state = EditorState.create({
    ...
    extensions: [
      ...
      history(),
      languageConfig.of(getLanguageExtension()),
  ...
  <div class="code-editor-wrapper" :class="{ 'is-fullscreen': isFullscreen }">
    <div class="code-editor-toolbar" @click.stop>
      <div class="toolbar-left">
        <span class="toolbar-title">{{ languageTitle }}</span>
      </div>
      <div class="toolbar-right">
        <button class="toolbar-btn" @click.stop.prevent="handleCopy" @mousedown.stop @mouseup.stop title="复制">
          <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
            <path
              d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z" />
          </svg>
        </button>
        <button class="toolbar-btn" @click.stop="handleFormat" @mousedown.stop @mouseup.stop title="格式化代码">
          <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
            <path d="M3 21h18v-2H3v2zm0-4h18v-2H3v2zm0-4h18v-2H3v2zm0-4h18V7H3v2zm0-6v2h18V3H3z" />
          </svg>
        </button>
        <button class="toolbar-btn" @click.stop="handleUndo" @mousedown.stop @mouseup.stop title="撤销">
          <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
            <path
              d="M12.5 8c-2.65 0-5.05.99-6.9 2.6L2 7v9h9l-3.62-3.62c1.39-1.16 3.16-1.88 5.12-1.88 3.54 0 6.55 2.31 7.6 5.5l2.37-.78C21.08 11.03 17.15 8 12.5 8z" />
          </svg>
        </button>
        <button class="toolbar-btn" @click.stop="handleRedo" @mousedown.stop @mouseup.stop title="重做">
          <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
            <path
        ...
        <button class="toolbar-btn" @click.stop.prevent="toggleFullscreen" @mousedown.stop @mouseup.stop
    ...
    <div v-if="isFullscreen" class="fullscreen-overlay" @click="exitFullscreen"></div>
  </div>
</template>
...
.code-editor-wrapper.is-fullscreen .code-editor {
