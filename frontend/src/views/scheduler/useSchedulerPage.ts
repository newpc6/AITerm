...
export function useSchedulerPage() {
  ...
  function openCreate() {
    editingId.value = null; form.value = { name: '', description: '', agent_id: agents.value[0]?.id || '', input_message: '', cron_expression: '0 8 * * *', max_retries: 0, timeout_seconds: 300 }
    dialogVisible.value = true
  }

  function openEdit(t: TaskItem) {
    editingId.value = t.id; form.value = { name: t.name, description: t.description, agent_id: '', input_message: '', cron_expression: t.cron_expression, max_retries: 0, timeout_seconds: 300 }
    dialogVisible.value = true
  }
  ...
  async function handleSave() {
    ...
    saving.value = true
    try {
      if (editingId.value) { await http.put(`/api/v1/scheduler/${editingId.value}`, form.value); ElMessage.success('更新成功') }
      else { await http.post('/api/v1/scheduler', form.value); ElMessage.success('创建成功') }
  ...
  async function handleDelete(id: string) {
    try { await ElMessageBox.confirm('确定删除吗？', '删除确认', { type: 'warning' }); await http.delete(`/api/v1/scheduler/${id}`); ElMessage.success('已删除'); await load() } catch { /* */ }
  }

  async function handleRun(id: string) {
    try { await http.post(`/api/v1/scheduler/${id}/run`); ElMessage.success('已触发执行') } catch { ElMessage.error('执行失败') }
  }

  async function showLogs(taskId: string) {
    logLoading.value = true; logDialogVisible.value = true
    try {
