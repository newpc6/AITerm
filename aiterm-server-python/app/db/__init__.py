...
async def rebuild_table(conn, model, existing_columns: Dict[str, Dict[str, Any]]):
    ...
    pk_columns = [col_name for col_name, col in model_columns.items() if col.primary_key]
    if not pk_columns:
        pk_columns = ["id"]
    ...
    for col_name, column in model_columns.items():
        ...
        create_cols.append(f"{col_name} {col_type} {nullable} {default} {pk_clause}".strip())
    
    create_sql = f"CREATE TABLE {temp_table_name} ({', '.join(create_cols)})"
    logger.info(f"Creating temp table: {temp_table_name}")
    await conn.execute(text(create_sql))
    
    common_columns = [col for col in model_columns.keys() if col in existing_columns]
            ...
            for col_name, column in model_columns.items():
                if col_name not in existing_columns:
                    ...
                    if not types_compatible(db_type, model_type):
                        logger.info(f"Column {col_name} type changed from {db_type} to {model_type}, will rebuild table")
                        needs_rebuild = True
                    ...
                    if column.default is not None and hasattr(column.default, 'arg'):
                        ...
                        if default_val is not None and default_val != "":
                            ...
                            try:
                                ...
                                logger.info(f"Updated default values for {col_name}")
                            except Exception as e:
                                logger.warning(f"Failed to update default values for {col_name}: {e}")
