    ...
    try:
        with open(path, "r", encoding=encoding) as f:
            if max_lines:
                ...
                content = f.read()
        return {"success": True, "path": path, "content": content, "size": os.path.getsize(path)}
...
''',
    ...
    try:
        ...
        with open(path, write_mode, encoding=encoding) as f:
            f.write(content)
        return {"success": True, "path": path, "size": os.path.getsize(path), "mode": mode}
    except Exception as e:
...
''',
    ...
    try:
        ...
        for item in os.listdir(path):
            ...
            items.append({"name": item, "type": "directory" if is_dir else "file", "size": 0 if is_dir else os.path.getsize(item_path)})
