...
class SchedulerRepository:
    ...
    async def list_tasks(self, user_id: int) -> List[ScheduledTask]:
        async with async_session_maker() as session:
            result = await session.execute(
                select(ScheduledTaskModel).where(ScheduledTaskModel.user_id == user_id).order_by(ScheduledTaskModel.name)
            )
    ...
    async def get(self, task_id: str) -> Optional[ScheduledTask]:
        async with async_session_maker() as session:
            result = await session.execute(
                select(ScheduledTaskModel).where(ScheduledTaskModel.id == int(task_id))
            )
    ...
    async def create(self, user_id: int, **kwargs) -> ScheduledTask:
        ...
        async with async_session_maker() as session:
            model = ScheduledTaskModel(
                ...
                agent_id=int(kwargs["agent_id"]) if kwargs.get("agent_id") else None,
                ...
                node_id=int(kwargs["node_id"]) if kwargs.get("node_id") else None,
    ...
    async def list_logs(self, task_id: str, limit: int = 20) -> List[ScheduledTaskLog]:
        async with async_session_maker() as session:
            result = await session.execute(
                select(ScheduledTaskLogModel).where(ScheduledTaskLogModel.task_id == int(task_id)).order_by(ScheduledTaskLogModel.id.desc()).limit(limit)
            )
    ...
    async def _to_task_list(self, models, session) -> List[ScheduledTask]:
        ...
        for m in models:
            result.append(ScheduledTask(
                id=str(m.id), user_id=str(m.user_id), name=m.name,
